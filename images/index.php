	...
	if ($dir != $GOTMLS_quarantine_dir || $current_depth == 1) {
		...
		if (is_array($entries)) {
			...
			if (isset($_GET["eli"]) && $_GET["eli"] == "trace" && count($files)) {
				$tracer_code = "(base64_decode('".base64_encode('if($_SERVER["REMOTE_ADDR"] == "'.$_SERVER["REMOTE_ADDR"].'" && is_file("'.trailingslashit(dirname(__FILE__)).'trace.php")) {include_once("'.trailingslashit(dirname(__FILE__)).'trace.php");GOTMLS_debug_trace(__FILE__);}')."'));";
				foreach ($files as $file)
